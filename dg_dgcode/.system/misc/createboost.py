...
#build select libs (specified here with inter-dependencies, None means it will be embedded in the library of the Core package):
...
for lm in sorted(lib_mods.keys()):
    ...
    if pkgdeps is not None:
        pkgdir = os.path.join(dgdir,'packages/Framework/CoreLibs/Boost%s'%lm.capitalize())
        utils.rm_rf(pkgdir)
        ...
        pkgsrc=os.path.join(dgdir,'packages/Framework/Core/libsrc')
