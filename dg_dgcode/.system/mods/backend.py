...
def perform_configuration(cmakeargs=[],
    ...
    pl = loadpkgs.PackageLoader(os.path.join(dirs.codedir),
                                possible_extdeps,
                                select_filter,
                                exclude_filter,
    ...
    if envdict:
        ...
        linkdir=os.path.abspath(os.path.realpath(os.path.join(dirs.installdir,'lib','links')))
        utils.rm_rf(linkdir)
    ...
    utils.mkdir_p(os.path.join(dirs.blddir,'langs'))
    for lang,info in envdict['system']['langs'].items():
        utils.update_pkl_if_changed(info,os.path.join(dirs.blddir,'langs',lang))
    ...
    def nuke_pkg(pkgname):
        import pipes
        conf.uninstall_package(pipes.quote(dirs.installdir),pipes.quote(pkgname))
    ...
    dgtestfile=os.path.join(dirs.blddir,'dgtests')
    ...
    cfgfile=os.path.join(dirs.blddir,'cfg.py')
