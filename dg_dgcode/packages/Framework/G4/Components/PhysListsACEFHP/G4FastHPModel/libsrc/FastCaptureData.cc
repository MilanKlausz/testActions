...
struct PrivateXsTable { typedef hacktype(G4NeutronHPCaptureData::*type); };
template struct rob<PrivateXsTable, &G4NeutronHPCaptureData::theCrossSections>;
...
FastCaptureData::FastCaptureData()
...
void FastCaptureData::GenerateHotData()
{
  G4PhysicsTable* table = this->*result<PrivateXsTable>::hacktype;
  if(!table)
    throw std::runtime_error("FastCaptureData::GenerateHotData table is not created\n");
  man->initCapture(table);
  G4cout << "FastCaptureData::GenerateHotData: hot data created" << G4endl;
}



G4double FastCaptureData::GetIsoCrossSection( const G4DynamicParticle* dp ,
    G4int /*Z*/ , G4int /*A*/ ,
    const G4Isotope* /*iso*/  ,
...
G4double FastCaptureData::GetCrossSection(const G4DynamicParticle* aP, const G4Element* anE, G4double aT)
{
  G4double eKinetic = aP->GetKineticEnergy();
  G4double xs=man->GetCaptureCrossSection( anE, eKinetic, aT);
