...
struct PrivateXsTable { typedef hacktype(G4NeutronHPElasticData::*type); };
template struct rob<PrivateXsTable, &G4NeutronHPElasticData::theCrossSections>;
...
FastElasticData::FastElasticData()
...
void FastElasticData::GenerateHotData()
{
  G4PhysicsTable* table = this->*result<PrivateXsTable>::hacktype;
  if(!table)
    throw std::runtime_error("FastElasticData::GenerateHotData table is not created\n");
  man->initElastic(table);
  G4cout << "FastElasticData::GenerateHotData: hot data created" << G4endl;
}



G4double FastElasticData::GetIsoCrossSection( const G4DynamicParticle* dp ,
    G4int /*Z*/ , G4int /*A*/ ,
    const G4Isotope* /*iso*/  ,
...
G4double FastElasticData::GetCrossSection(const G4DynamicParticle* aP, const G4Element* anE, G4double aT)
{
  G4double eKinetic = aP->GetKineticEnergy();
  G4double xs=man->GetElasticCrossSection( anE, eKinetic, aT);
