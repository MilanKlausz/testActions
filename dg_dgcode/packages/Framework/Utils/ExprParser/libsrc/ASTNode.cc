...
namespace ExprParser {
  ...
  bool ExprEntityBase::isConstant() const
  {
    ...
    for (auto it = children().begin();it!=children().end();++it) { auto& c = *it;
#else
  ...
  public:
    ...
    virtual type_res evaluate() const {
      return _eval<type_arg>(ExprEntityBase::child(0));
    }
  };
...
#ifdef NEED_CXX11_WORKAROUNDS
    return std::to_string((long long int)_eval<int_type>(ExprEntityBase::child(0)));
#else
...
#ifdef NEED_CXX11_WORKAROUNDS
    return std::to_string((long double)_eval<float_type>(ExprEntityBase::child(0)));
#else
