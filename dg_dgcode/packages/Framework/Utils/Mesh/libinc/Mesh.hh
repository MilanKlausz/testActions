...
private:
...
#ifndef NEED_CXX11_WORKAROUNDS
    void
#else
    Mesh_FakeLambda *
#endif
    openFile(const std::string&, TDataProvider &, gzFile&);
  ...
  Mesh_FakeLambda *
#endif
  Mesh<NDIM>::openFile(const std::string& filename, TDataProvider & dp, gzFile& file)
  {
    assert(!dp&&!file);
    ...
    Mesh_FakeLambda * fakelambda = new Mesh_FakeLambda(file);
    dp = std::bind(&Mesh_FakeLambda::doRead,fakelambda,std::placeholders::_1,std::placeholders::_2);
    return fakelambda;
#endif
  }
...
#else
    std::auto_ptr<Mesh_FakeLambda> fake(openFile(filename,dp,file));
#endif
    reinit(dp);
    gzclose(file);
  }
...
#else
    //workaround for incomplete c++11 support
    Mesh_FakeLambda fakelambda(file);
    TDataAcceptor da = std::bind(&Mesh_FakeLambda::doWrite,&fakelambda,std::placeholders::_1,std::placeholders::_2);
#endif
    write(da);
    gzclose(file);
  }
...
#else
    std::auto_ptr<Mesh_FakeLambda> fake(openFile(filename_other,dp,file));
#endif
    Mesh other;
